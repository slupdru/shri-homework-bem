...
const Menu = ({dispatch, activePage, state}: {state: State; dispatch: any; activePage: number}) => {
  ...
  {menuLinksArr.map((el, index) => {
    ...
    return <a
  onClick={() => {
    dispatch(getChangeActivePageAction(index));
  }}
  key={index}
  href={el.href}
  ...
  <select onChange={(event) => {onChangeFields('types', event.currentTarget.value); }} defaultValue="all" className={cnMenu('Selector')} id="type">
    <option value="info">info</option>
    <option value="critical">crirical</option>
    <option value="all">all</option>
  </select>
  <div className={cnMenu('Selector')}><span> Страница</span> <input onInput={(event) => {onChangeFields('page', event.currentTarget.value); }}defaultValue="1" id="page" type="text"/></div>
  <div className={cnMenu('Selector')}><span>Количество на странице</span> <input onInput={(event) => {onChangeFields('itemsPerPage', event.currentTarget.value); }} defaultValue="10" id="quantity" type="text"/></div>
  </div>
</nav>;
};
